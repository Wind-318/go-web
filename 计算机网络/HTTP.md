...
- ### HTTP 报文(图片来源于网络)
  ![](http首部.jpg)
- ### HTTP 请求与响应
  - **请求报文**：发送一个 http 请求包含以下格式：请求行、请求头部、空行和请求数据。  
    **请求头示例**：
    ```
...
- ### URL：统一资源定位符，用来定位一个资源，URL 由协议名、域名、端口号组成。例：https://cn.bing.com/
  - #### 输入 URL 到页面加载发生了什么？
    ...
    - DNS 解析拿到 IP 地址
      ...
      - 迭代查找，查找根服务器、查找顶级域、二级域、三级域，直到找到。
      - 本地 DNS 找到后返回 IP，然后将其缓存以便下次查找
    - TCP 连接（三次握手）
    - 发送 HTTP 请求
    - 数据包传递到下层协议栈处理，在传输层被加上 TCP 头，在网络层被加上 IP 头
    - 服务器处理请求
    - 浏览器获得 html 文件，浏览器渲染页面，构建 DOM 树，执行 js 代码请求到更多文件。
    - 断开连接（四次挥手）
- ### 请求方法
  - #### GET：向服务器获取资源
  - #### HEAD：只返回响应头的 GET 请求
  - #### POST：向服务器提交表单数据
  - #### GET 请求和 POST 请求的区别
    - GET 请求将数据放在 URL 中，POST 放在表中。
    - GET 请求 HTTP 协议不作长度限制，只是一般情况下**浏览器**会对 URL 长度作限制导致发送的限制。
    - POST 请求发送数据采用表的形式，GET 请求会将请求头与数据一起发送过去。GET 请求刷新无害，
    - POST 请求刷新会重新提交数据。
    - GET 请求和 POST 请求都不安全，因为 HTTP 是明文传输，想保证安全性要使用 HTTPS。
- ### 常见 HTTP 状态码
    | 状态码 | 描述 |
    | :---: | :---: |
    | 1xx  | 请求正常处理 |
...
- ### HTTP 与 HTTPS 的区别
  ...
  - #### 使用 HTTPS 要在 CA(Certificate Authority, 数字证书认证机构) 申请证书。任何客户端都可以获得 CA 的证书（含公钥）用以验证它的证书。
  ...
  - #### HTTPS 工作流程
    ...
    - 客户端用会话密钥解密服务端的数据
  - #### 客户端如何验证证书？
    - 服务器生成非对称加密秘钥后，保留私钥，将公钥发送给 CA 进行签名，CA 会预先生成非对称秘钥，私钥对服务器的公钥签名生成 CA 证书，然后将证书给服务器，服务器再将公钥（含证书）发送给客户端，客户端通过得到的 CA 公钥进行验证。
  - #### 中间人攻击
    - SSL 劫持攻击：攻击者将自己接入到客户端和服务器之间，伪造服务器的证书以获得数据。证书校验可以预防劫持。
    - SSL 剥离攻击：攻击者在劫持 HTTP 会话后将 HTTP 页面中的所有 https:// 替换为 http://，用户访问时便使用 http 访问。
...
- ## I/O 多路复用
  ...
  - ### 最小轮询
    - 只处理准备好了的 I/O 事件。
  - ### select
    - 采用无差别轮询，进程一次处理多个 I/O 请求，select 会轮询所有请求，然后找到就绪的请求并处理。每次处理都需要拷贝数据到内核态。且最大连接数限制为 2048（64位）。
  - ### poll
    - 使用无差别轮询，poll 是链式的，无最大连接数限制，其它与 select 类似。
  - ### epoll
    - 使用最小轮询，与 select 相比不限制监视对象的数量，I/O 效率不会因监视数量增多而下降，效率为 $O(1)$。epoll 在事件注册时就拷贝进内核，所以不需要每次都拷贝。
    - 边缘触发(ET)：缓冲区从不可读变为可读才会唤醒进程，缓冲区内容变少则不会。
    - 水平触发(LT)：只要缓冲区中有数据就会一直返回描述符就绪状态，不断唤醒进程。